	...
	def valid_moves(self, game, piece_file, piece_rank):
		...
		for move in move_set:
			...
			if not target.code or (target.code and self.is_opponent(target)):
				moves.append(move)
		# check for castling
		moves += self.can_castle(game, piece_file, piece_rank)
		return moves
	
	def checkable(self, game, piece_file, piece_rank):
		...
		if self.color == 0:
			...
			for blk_piece in game.board.blk_pieces:
				piece = game.board.data[blk_piece[1]][blk_piece[0]]
				ms = piece.valid_moves(game, blk_piece[1], blk_piece[0])
				attacks += ms
		elif self.color == 1:
			# black is current player; generate the attack surface of white's pieces
			for wht_piece in game.board.wht_pieces:
				piece = game.board.data[wht_piece[1]][wht_piece[0]]
				ms = piece.valid_moves(game, wht_piece[1], wht_piece[0])
				attacks += ms
		attacks = list(set(attacks))
		return (piece_rank, piece_file) in attacks

	def can_castle(self, game, piece_file, piece_rank):
		...
		if self.color == 0:
			...
			if current_piece.code == 5 and not current_piece.has_moved and not current_piece.in_check:
				# white king
				k_rook = game.board.data[piece_rank][piece_file + 3]
				q_rook = game.board.data[piece_rank][piece_file - 4]
				if k_rook.code == 1 and not k_rook.has_moved and not k_rook.promoted:
					...
					if not game.board.data[piece_rank][piece_file + 1].code and not game.board.data[piece_rank][piece_file + 2].code:
						if not self.checkable(game, piece_file + 1, piece_rank) and not self.checkable(game, piece_file + 2, piece_rank):
							castles += [(piece_rank, piece_file + 2)]
				if q_rook.code == 1 and not q_rook.has_moved and not q_rook.promoted:
			...
			if current_piece.code == 11 and not current_piece.has_moved and not current_piece.in_check:
				...
				k_rook = game.board.data[piece_rank][piece_file + 3]
				q_rook = game.board.data[piece_rank][piece_file - 4]
				if k_rook.code == 7 and not k_rook.has_moved and not k_rook.promoted:
					# check spaces between king and kingside rook
					if not game.board.data[piece_rank][piece_file + 1].code and not game.board.data[piece_rank][piece_file + 2].code:
						if not self.checkable(game, piece_file + 1, piece_rank) and not self.checkable(game, piece_file + 2, piece_rank):
							castles += [(piece_rank, piece_file + 2)]
				if q_rook.code == 7 and not q_rook.has_moved and not q_rook.promoted:
					# check spaces between king and queenside rook
					if not game.board.data[piece_rank][piece_file - 1].code and not game.board.data[piece_rank][piece_file - 2].code and not game.board.data[piece_rank][piece_file - 3].code:
						if not self.checkable(game, piece_file - 1, piece_rank) and not self.checkable(game, piece_file - 2, piece_rank):
							castles += [(piece_rank, piece_file - 2)]
