...
class Queen(Piece):
	...
	def get_proximity(self, piece_file, piece_rank):
		return [
			...
			[(piece_rank, piece_file - 1), (piece_rank, piece_file - 2), (piece_rank, piece_file - 3), (piece_rank, piece_file - 4), (piece_rank, piece_file - 5), (piece_rank, piece_file - 6), (piece_rank, piece_file - 7)],
			[(piece_rank, piece_file + 1), (piece_rank, piece_file + 2), (piece_rank, piece_file + 3), (piece_rank, piece_file + 4), (piece_rank, piece_file + 5), (piece_rank, piece_file + 6), (piece_rank, piece_file + 7)],
			[(piece_rank - 1, piece_file), (piece_rank - 2, piece_file), (piece_rank - 3, piece_file), (piece_rank - 4, piece_file), (piece_rank - 5, piece_file), (piece_rank - 6, piece_file), (piece_rank - 7, piece_file)],
			[(piece_rank + 1, piece_file), (piece_rank + 2, piece_file), (piece_rank + 3, piece_file), (piece_rank + 4, piece_file), (piece_rank + 5, piece_file), (piece_rank + 6, piece_file), (piece_rank + 7, piece_file)],
			# diagonal moves
	...
	def valid_moves(self, board, piece_file, piece_rank):
		...
		moves.append(list(filter(board.in_bounds, move_set[4])))
		moves.append(list(filter(board.in_bounds, move_set[5])))
		moves.append(list(filter(board.in_bounds, move_set[6])))
		moves.append(list(filter(board.in_bounds, move_set[7])))
		return moves
